    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-blue-800 mb-1">Total Consumed</h3>
                <p className="text-2xl font-bold text-blue-600">{totalConsumed.toLocaleString()}</p>
                <p className="text-xs text-blue-500 mt-1">tokens used</p>
            </div>

            <div className="bg-green-50 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-green-800 mb-1">Total Purchased</h3>
                <p className="text-2xl font-bold text-green-600">{totalPurchased.toLocaleString()}</p>
                <p className="text-xs text-green-500 mt-1">tokens added</p>
            </div>

            {mostUsedFeature.length > 0 ? (
                <div className="bg-purple-50 p-4 rounded-lg">
                    <h3 className="text-sm font-medium text-purple-800 mb-1">Most Used Feature</h3>
                    <p className="text-2xl font-bold text-purple-600">{formatFeatureName(mostUsedFeature[0])}</p>
                    <p className="text-xs text-purple-500 mt-1">{mostUsedFeature[1].toLocaleString()} tokens</p>
                </div>
            ) : (
                <div className="bg-purple-50 p-4 rounded-lg">
                    <h3 className="text-sm font-medium text-purple-800 mb-1">Most Used Feature</h3>
